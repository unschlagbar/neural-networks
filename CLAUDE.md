...
- **Chat special tokens.** `SPECIAL_TOKENS` gains `<CONTEXT>` (258) and `<SEP>` (259) *after* the pretraining `<W>`/`<END>`, so no existing byte/`<W>`/`<END>` id ever shifts. A **tool result** needs no token of its own: `Role::Tool` renders as the plain text `<result>…</result>` followed by `<SEP>`, so it sits on the prompt side exactly like a user turn and the reply after it is what carries loss. The tool syntax is a convention of the data alone (`mixes/synth/*.syn`), so changing the tool protocol never costs a vocab entry or a checkpoint migration. `vocab_size()` is therefore 260, while a pretrained checkpoint has 258 (`PRETRAIN_SPECIALS`). The pretraining `<END>` (257) doubles as the SFT end-of-response target/stop.
