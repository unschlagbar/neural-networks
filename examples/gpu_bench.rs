...
fn main() {
    ...
    println!("{:>16} {:>7} {:>11} {:>11} {:>9}", "M,K,N", "iters", "CPU GF/s", "GPU GF/s", "speedup");
    for &(m, k, n) in &[(256, 256, 256), (512, 512, 512), (1024, 1024, 1024), (2048, 1024, 1024)] {
    ...
    // Transfer-cost illustration on the 1024³ case.
    {
        ...
        let db = DTensor::from_host(&gpu, &b);
        let mut dc = DTensor::zeros(&gpu, &[m, n]);
        let resident = gpu_time(&gpu, 3, iters, || ops::matmul_nn_into(&gpu, &da, &db, &mut dc, 0.0));
    ...
    println!("{:>16} {:>7} {:>11} {:>11} {:>9}", "B,in,out", "iters", "CPU GF/s", "GPU GF/s", "speedup");
    for &(batch, inp, out) in &[(512, 512, 512), (2048, 1024, 1024), (4096, 1024, 2048)] {
        ...
        println!(
            "{:>6},{:>4},{:>4} {:>7} {:>11.1} {:>11.1} {:>8.1}x",
            batch, inp, out, iters, cpu_g, gpu_g, gpu_g / cpu_g
        );
    }
    ...
    println!("{:>16} {:>7} {:>11} {:>11} {:>9}", "B,T,H", "iters", "CPU GF/s", "GPU GF/s", "speedup");
    for &(b, t, h) in &[(32, 32, 256), (32, 64, 512), (64, 64, 512)] {
        ...
        let (cpu_g, gpu_g) = (gflops(flops, CPU_ITERS, cpu_s), gflops(flops, GPU_ITERS, gpu_s));
        println!(
            "{:>6},{:>3},{:>4} {:>7} {:>11.1} {:>11.1} {:>8.1}x",
            b, t, h, GPU_ITERS, cpu_g, gpu_g, gpu_g / cpu_g
        );
    }
    ...
    println!("{:>18} {:>7} {:>13} {:>13} {:>9}", "B,T,d,heads,dqk", "iters", "CPU ms/it", "GPU ms/it", "speedup");
    for &(b, t, d, heads, dqk) in &[(16, 32, 256, 4, 32), (16, 64, 512, 8, 32), (32, 64, 512, 8, 64)] {
        ...
        let mut dev = neural_networks::gpu::mlstm::MLstm::from_parts(
            &gpu, d, d, heads, dqk,
            &cpu.wq, &cpu.wk, &cpu.wv, &cpu.wo, &cpu.wi, &cpu.wf,
            &cpu.bq, &cpu.bk, &cpu.bv, &cpu.bo, &cpu.bi, &cpu.bf,
            &cpu.w_out, &cpu.b_out, &cpu.gamma,
        );
        let dx_in = DTensor::from_host(&gpu, &x);
        let ddy = DTensor::from_host(&gpu, &dy);
        let gpu_s = gpu_time(&gpu, GPU_WARMUP, GPU_ITERS, || {
        ...
        println!(
            ...
            b, t, d, heads, dqk, GPU_ITERS, cpu_ms, gpu_ms, cpu_ms / gpu_ms
        );
    }
    ...
    println!("{:>20} {:>7} {:>13} {:>13} {:>9}", "vocab,B,T,H,up", "iters", "CPU ms/it", "GPU ms/it", "speedup");
    for &(vocab, b, t, hidden, up, heads, dqk) in &[
        ...
        let blocks: Vec<Box<dyn BlockLike>> = vec![
            Box::new(Block::<SLstm>::from_cpu(&gpu, &SLstmBlock::new_slstm(hidden, up))),
            Box::new(Block::<MLstm>::from_cpu(&gpu, &MLstmBlock::new_mlstm(hidden, up, heads, dqk))),
        ];
        let mut dev = Lm::from_parts(
            &gpu, &Tensor::random(&[vocab, hidden], 0.1), blocks,
            &Tensor::new(&[hidden], vec![1.0; hidden]),
            &Tensor::xavier(hidden, vocab), &Tensor::zeros(&[vocab]), cap,
        );
        let gpu_s = gpu_time(&gpu, GPU_WARMUP, GPU_ITERS, || {
        ...
        println!(
            ...
            vocab, b, t, hidden, up, GPU_ITERS, cpu_ms, gpu_ms, cpu_ms / gpu_ms
        );
    }
}
