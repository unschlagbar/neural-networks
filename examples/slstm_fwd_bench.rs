//! sLSTM FORWARD only, at the four shapes the hierarchical model actually runs it
...
//!
//! Reports the BEST of `REPS` timed rounds rather than the mean: the SM clock on
//! this card swings between ~1.4 and ~2.9 GHz, which is wider than most wins being
//! measured, and the best round is the one closest to a fixed clock.
//!
//! `issue` is the same region without the trailing sync, i.e. host submission time
//! alone. `issue / wall` near 1 means the card is waiting on the driver and the fix
//! is to launch less; well below 1 means the kernels are what to optimize.
//!
//!   cargo run --release --features cuda --example slstm_fwd_bench
...
fn main() {
    ...
    for (label, b, t, h) in shapes {
        ...
        for _ in 0..reps {
            ...
            let issue = t0.elapsed().as_secs_f64() / iters as f64;
            gpu.stream.synchronize().unwrap();
            let wall = t0.elapsed().as_secs_f64() / iters as f64;
            best_wall = best_wall.min(wall);
            best_issue = best_issue.min(issue);
        }
        println!(
