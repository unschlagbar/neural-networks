...
fn main() {
    let lens: Vec<usize> = {
        ...
        if a.is_empty() { vec![32, 128, 512, 2048] } else { a }
    };
    ...
    let steps: usize = std::env::var("STEPS").ok().and_then(|s| s.parse().ok()).unwrap_or(0);
    ...
    let corpus: Option<Vec<(Vec<usize>, Vec<Range<usize>>)>> = std::env::var("CORPUS")
        .ok()
        .map(|path| {
            ...
            let skip: usize = std::env::var("SKIP").ok().and_then(|s| s.parse().ok()).unwrap_or(0);
            chunk
                .iter()
                .skip(skip)
                .take(steps.max(1))
                .map(|b| (b.tokens.iter().map(|&t| t as usize).collect(), b.words.clone()))
                .collect()
        });
    ...
    for &nw in &lens {
        ...
        for s in 0..steps {
            ...
            let ml: f32 = norms.iter().enumerate().filter(|(i, _)| i % 4 != 0).map(|(_, v)| v).sum();
            ...
            if norms.iter().any(|v| !v.is_finite()) {
                ...
                for stage in ["encoder", "backbone", "decoder"] {
                    ...
                    let bad: Vec<usize> =
                        n.iter().enumerate().filter(|(_, v)| !v.is_finite()).map(|(i, _)| i).collect();
...
fn report(model: &Hierarchical, gpu: &Gpu) {
    for (i, gn) in model.grad_norms_by_block(gpu, "backbone").iter().enumerate() {
