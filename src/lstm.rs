...
impl LSTMLayer {
    ...
    pub fn validate_cache(&mut self, cache: &LSTMCache) {
        ...
        assert_eq!(cache.dh.len(), hidden_size);
        assert_eq!(cache.dc.len(), hidden_size);
        assert_eq!(cache.dh_prev.len(), hidden_size);
        assert_eq!(cache.dc_prev.len(), hidden_size);
    ...
    pub fn forward(&mut self, input: &[f32], cache: &mut LSTMCache) {
        ...
        cache.dh_prev.copy_from_slice(&self.dh);
        cache.dc_prev.copy_from_slice(&self.dc);
        let dh_prev: &[f32] = &cache.dh_prev;
        let dc_prev: &[f32] = &cache.dc_prev;

        // Combine input + previous hidden state
        let xh = &mut cache.xh;
        xh[..input.len()].copy_from_slice(input);
        xh[input.len()..].copy_from_slice(dh_prev);
        ...
        let ft: &mut [f32] = &mut cache.ft;
        let it: &mut [f32] = &mut cache.it;
        let ct: &mut [f32] = &mut cache.ct;
        let ot: &mut [f32] = &mut cache.ot;
        ...
        let prev_c = dc_prev;
        for j in 0..h {
            self.dc[j] = ft[j] * prev_c[j] + it[j] * ct[j];
        }

        // Compute hidden state: h_t = o_t * tanh(c_t)
        debug_assert_eq!(ot.len(), self.dc.len());
        for (j, &ot) in ot.iter().enumerate() {
            self.dh[j] = ot * self.dc[j].tanh();
        }

        cache.dh.copy_from_slice(&self.dh);
        cache.dc.copy_from_slice(&self.dc);
    ...
    pub fn backwards(
        ...
        Activation::Tanh.activate(&mut cache.dc);

        let do_ = &mut cache.do_;
        let df = &mut cache.df;
        let di = &mut cache.di;
        let dct = &mut cache.dct;

        // 2) dc = dh ⊙ o * (1 - tanh(c)^2) + dc_next
        let dc = dc_next;
        for i in 0..h {
            dc[i] += delta_h[i] * cache.ot[i] * dtanh_from_y(cache.dc[i]);
        }

        // 3) do = dh ⊙ tanh(c) * σ'(o)
        for i in 0..h {
            do_[i] = delta_h[i] * cache.dc[i] * dsigmoid_from_y(cache.ot[i]);
        }

        // 4) gate grads: df, di, dct
        for i in 0..h {
            df[i] = dc[i] * cache.dc_prev[i] * dsigmoid_from_y(cache.ft[i]);
    ...
    pub fn make_cache(&self) -> LSTMCache {
        ...
        LSTMCache {
            xh: vec![0.0; rows].into(),
            //states_prev: Matrix::zeros(2, hidden_size),
            dh_prev: vec![0.0; hidden_size].into(),
            dc_prev: vec![0.0; hidden_size].into(),
            ft: vec![0.0; hidden_size].into(),
            it: vec![0.0; hidden_size].into(),
            ct: vec![0.0; hidden_size].into(),
            ot: vec![0.0; hidden_size].into(),
